...
import { X, UserPlus, LogIn, Lock, User as UserIcon, Mail, Smile } from 'lucide-react';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isRegister) {
        ...
        if (!user) {
          setError('Benutzername nicht gefunden. Bitte überprüfe deine Eingabe.');
          setLoading(false);
          return;
        }
      ...
      <div className="bg-[#FDFCF8] w-full max-w-sm rounded-3xl p-6 shadow-2xl border border-[#E0D8CC] animate-in zoom-in-95 duration-150 relative">
        ...
        <div className="text-center mb-5">
          ...
          <p className="text-xs text-[#64748b] mt-0.5">
            {isRegister ? 'Anzeigename frei wählbar. Sofort loslegen!' : 'Mit deinem Benutzernamen anmelden'}
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          <div>
            ...
            <div className="relative">
              ...
              <input
                ...
                placeholder="z. B. timo356"
                className="w-full bg-white border border-[#E0D8CC] focus:border-[#D46A43] rounded-2xl pl-9 pr-3.5 py-2 text-sm text-[#2D3047] outline-none"
              />
            </div>
          </div>
          ...
          {isRegister && (
            <div>
              ...
              <div className="relative">
                <Smile className="w-4 h-4 absolute left-3 top-1/2 -translate-y-1/2 text-[#94a3b8]" />
                <input
                  type="text"
                  value={displayName}
                  onChange={(e) => setDisplayName(e.target.value)}
                  placeholder="z. B. Chef Timo"
                  className="w-full bg-white border border-[#E0D8CC] focus:border-[#D46A43] rounded-2xl pl-9 pr-3.5 py-2 text-sm text-[#2D3047] outline-none"
                />
              </div>
            </div>
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full bg-[#D46A43] hover:bg-[#c05a38] text-white font-bold py-3 rounded-2xl text-xs shadow-md transition-all mt-2 flex items-center justify-center gap-2 disabled:opacity-50"
          >
            {loading ? (
